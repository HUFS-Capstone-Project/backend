...
final class LinkUrlNormalizer {
	...
	static NormalizedUrl normalize(String rawUrl) {
		if (rawUrl == null || rawUrl.isBlank()) {
			throw new FieldValidationException("originalUrl", "URL은 필수입니다.");
		}
		String candidate = rawUrl.trim();
		if (candidate.length() > MAX_URL_LENGTH) {
			throw new FieldValidationException("originalUrl", "URL 길이가 너무 깁니다.", rawUrl);
		}
		...
		try {
			...
			throw new FieldValidationException("originalUrl", "URL 형식이 올바르지 않습니다.", rawUrl);
		}
	...
	private static void validateScheme(String scheme) {
		if (scheme == null || scheme.isBlank()) {
			throw new FieldValidationException("originalUrl", "URL 스킴은 필수입니다.");
		}
		if (!ALLOWED_SCHEMES.contains(scheme.toLowerCase(Locale.ROOT))) {
			throw new FieldValidationException("originalUrl", "http/https URL만 허용합니다.", scheme);
		}
	}

	private static void validateHost(String host) {
		if (host == null || host.isBlank()) {
			throw new FieldValidationException("originalUrl", "URL 호스트는 필수입니다.");
		}
	}
