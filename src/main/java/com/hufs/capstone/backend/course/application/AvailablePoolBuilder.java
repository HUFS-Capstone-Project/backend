...
class AvailablePoolBuilder {
	...
	private final DateCourseCandidateQueryPort candidateQueryPort;
	...
	AvailablePool build(Long roomId, List<CategorySlotCommand> slots, Instant startDateTime, String sigunguCode) {
		...
		List<DateCourseCandidate> rows = candidateQueryPort.findCandidates(roomId, slots, now, sigunguCode);
		if (rows.isEmpty()) {
