...
public class MeDateCourseController implements MeDateCourseApi {
	...
	@Override
	public CommonResponse<MyDateCourseCursorPageResponse> listMyDateCourses(
			@RequestParam(required = false) Integer limit,
			@RequestParam(required = false) String cursor
	) {
		Long userId = SecurityUtils.currentUserIdOrThrow();
		return CommonResponse.ok(
				MyDateCourseCursorPageResponse.from(queryService.listMySavedCourses(userId, limit, cursor))
		);
	}
}
